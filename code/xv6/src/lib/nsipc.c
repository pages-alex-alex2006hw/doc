...
/* virtual address at which to receive page mappings containing client 
   requests */
...
/**
   ...
   @param request code, passed as the simple integer IPC value.
   @param fsreq page to send containing additional request data, usually 
            fsipcbuf; can be modified by server to return additional response 
            info
   @param dstva virtual address at which to receive reply page, 0 if none
   @param perm permissions of received page
   @return 0 if successful, < 0 on failure
*/
static int nsipc(unsigned type, void *fsreq, void *dstva, int *perm)
{
  envid_t whom;
