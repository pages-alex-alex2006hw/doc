...
\bit
\w Provides a \bb{namespace for devices, processes, properties, events, functions, and reactors}.

\vspace*{0.2cm}

\w Provides a \bb{language} for programming devices and workflows over devices.

\vspace*{0.2cm}

\w Provides a \bb{runtime} for dynamically changing device behavior and
...
\bb{hierarchical names}, which are finite strings: e.g.
\verb+person.cj.fitbit+. 

\vspace*{0.5cm}

The system allows \bb{registration of device names}.
   \bit
...
The event namespace consists of persistent \bb{hierarchical names}: e.g.
...
\bit
\w \bb{properties}: pull-based data, which is presented to outside
upon request 
\w \bb{events}: push-based data, which is sent to outside
\w \bb{functions}: pull-based computation, which is invoked from outside
\w \bb{reactors}: computation which is invoked upon event occurrence
\eit
