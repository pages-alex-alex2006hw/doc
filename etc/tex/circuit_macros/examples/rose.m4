...
ifdef(`pstricks_',`command "\definecolor{`gold'}{rgb}{gold_}"',
`ifdef(`mpost_',`define(`gold',`(gold_)')')')
...
C: (0,0)
  ...
  for i=0 to 15 do {
    ...
    for j=-1 to 1 by 2 do {
      {point_(sector*(i+j*0.5)) line to rvec_(outer_circle-spoke_tail,0) }
      { point_(sector*i)
  ...
  for i=0 to 15 do {
    ...
    for j = -1 to 1 by 2 do { move to X+vec_(0.20,j*0.14)
      point_(sector*(i+j*0.25)) line to rvec_(1.5*stripwid,0) }
  ...
  for i=0 to 15 do {
    ...
    for j=-1 to 1 by 2 do {
      ...
      point_(sector*(i+j*0.25)) line from X to X + vec_(outerroserad,0)
...
.PE
')dnl
ifdef(`pstricks_',`dorose',
`ifdef(`mpost_',`dorose',
`ifdef(`postscript_',`dorose',
`This diagram requires PSTricks, postscript, or MetaPost output')')')
